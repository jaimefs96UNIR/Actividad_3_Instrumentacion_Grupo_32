...
// Para que el usuario controle la luz de la cabina

int userLUZ = 0;
bool irLUZ = false;
bool masLUZ = false;

// Acción manual del enfriado
...
void controlarTemperatura() {
  ...
  Serial.println(deacLED);
  if (deacLED){
    switch(useManualCTRL){
      case 0:
  ...
  }else if (temperaturaControlada > limiteSuperior) {
    accionTemperatura = "Enfriar";
    mostrarRGBEnfriar();
...
void controlarIluminacion() {
  ...
  if (irLUZ){
    potenciaLuzArtificial = userLUZ;

  } else if (luzMedida < limiteInferior) {
...
void moverAscensorA(int destino) {
  ...
  // Espera mientras haya obstáculo
  distanciaCM = leerDistanciaCM();
...
void comprobarMandoIR() {
  if (IrReceiver.decode()) {
    ...
    switch (comando) {
      ...
      case 56:    // tecla 5 -> con esta tecla puedo encender y aumentar de intensidad la iluminación
        if (!masLUZ){
          userLUZ = 80;
          masLUZ = true;
        }else{
      ...
      case 90:   // tecla 6
        ...
        //escribirLedLuz(80);
        break;
      case 66: // Enfriado manual
        useManualCTRL = 0;
        deacLED = true;
        break;
      case 74: // Calentado manual
        useManualCTRL = 1;
        deacLED = true;
        break;
      case 82: // Desactivar modo manual
        useManualCTRL = 3;
        deacLED = false;
        break;
      default:
